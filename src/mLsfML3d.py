...
def mGetFeatures(image, sWord) :
    ...
    global bHandLeftVisiblePrev, bHandRightVisiblePrev, aDisplResult
    ...
    if holisticResults.pose_landmarks :
        ...
        # bde test - how many attempt <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
        if False and (((nLoopLeft > 1) and (nLoopLeft <= nAttemptFeat)) or ((nLoopRight > 1) and (nLoopRight <= nAttemptFeat))) :
            print(f"... hands found round {nLoopRight} {nLoopLeft} for {sWord} ")
        ...
        # bde test - hand.landmarks vs pose.wrist.visible ----------------------
        if False :
            print(len(aHandLeft), nHandLeftVisible, bHandLeftVisible, len(aHandRight), nHandRightVisible, bHandRightVisible)
    ...
    if ("hands" in aDisplay) or ("pose" in aDisplay) or ("face" in aDisplay) :
        ...
        nIdxLine = 1
        for sWordL, nProba, nWord in aDisplResult :
            sPredictWord = f"{nProba:4.1f}% {unidecode.unidecode(sWordL)}"
            cv2.putText(image, sPredictWord, (0, 45*nIdxLine), nFont, nFontScale, (0, 0, 255), nThickness, nLineType)
            nIdxLine += 1
            #if nIdxLine > 2 : break
            if nIdxLine > 3 : break
...
def mNormalizeFeatures(aHandLeft, aHandRight, aPoses, aFaces) :
    ...
    # bde test - cas no feature mormalize --------------------------------------
    if False : return aHandLeft, aHandRight, aPoses, aFaces
...
def mLsfFeature(sDirIn, sSrc, sDirModel, sDataFrame, mp_holistic, holistic) :
    ...
    for aFileSequence in aFileSequences :
        ...
        for aHandLeft, aHandRight, aPoses, aFaces, (bHandLeftAppear, bHandLeftDisAppear, bHandRightAppear, bHandRightDisAppear) in aVideoSequence :
            ...
            if bExtrapolateHands :
                if (len(aHandLeft) == 0) and (bHandLeftAppear or bHandLeftDisAppear) :
                    ...
                    # bde test -------------------------------------------------
                    if False : print("... aHandLeftRest", bHandLeftAppear, bHandLeftDisAppear)
                if (len(aHandRight) == 0) and (bHandRightAppear or bHandRightDisAppear) :
                    aHandRight = aHandRightRest
                    # bde test -------------------------------------------------
                    if False : print("... aHandRightRest", bHandRightAppear, bHandRightDisAppear)
        ...
        if (False) : print(f"... Missing;{sFile};{nHandEmpty}")
        if (False) :
            nHandTotal = 0
            for aSequence in aSequences :
...
def mLsfSelection(sDirModel, sDataFrame):
    ...
    # bde test - Analyse limited to X% of features <<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    nFeatureVector = sum(aShapes) * nSample
    nFeature = int(0.25 * nFeatureVector)
    if nFeature > 256 :
        nFeaturePrint = 256
    else :
        nFeaturePrint = nFeature
    #print("... nFeatureVector, nFeature, nFeaturePrint : ", nFeatureVector, nFeature, nFeaturePrint)
...
def mLsfTrainTest(sDirModel, sDataFrame):
    ...
    for sNormeName, oScaler in aNormalizers.items():
        ...
        for sClassifierName, oclassifier in aClassifiers.items():
            ...
            if False :
                aYPred = oclassifier.predict(aXTest)

                """
                # Top-k Accuracy classification score
                nK = 2
                print("... top " + str(nK) + " : ", top_k_accuracy_score(aYTest, score, k=nK))
                """

                # Confusion matrice and classification report ------------------
                cm = confusion_matrix(aYTest, aYPred, labels=oclassifier.classes_)
                disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=oclassifier.classes_)
                disp.plot()
                sFileNamePlot = './log/'+sSrc+'-'+sClassifierName+'-'+sNormeName+'.png'
                plt.savefig(sFileNamePlot, bbox_inches='tight')
                #plt.show()                
                
                cr = classification_report(aYTest, aYPred)
                print("... classification_report : ", cr)
                
                """
                # Display ROC curve
                oclassifier_disp = RocCurveDisplay.from_estimator(oclassifier, aXTest, aYTest)
                plt.show()                
                """
...
def mLsfMostFreq(nMostFreq, aWord, aPredict, bPrint = False) :
    ...
    if bPrint :
        ...
        for sWordL, nProba, nWord in aResult :
            print(f"...... {sWordL:24} {nProba:4.1f}% {nWord}x")
...
def mLsfContinue() :
    ...
    while (True) :
        ...
        while (True) :
            ...
            sWord, nConf = mLsfPrediction(aSeq)
        ...
        aSequences = aSequences[sWinSkip]
    ...
    for nIdxSign in range(len(aYInits)) :
        ...
        for sNormeName, oScaler in aNormalizers.items():
            ...
            for sClassifierName, oclassifier in aClassifiers.items():
                ...
                # bde test <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
                if False : print(f"... {sPreviousWord:16} + {sClassifierName:8} + {sNormeName:8} => {sWordPredict:16} à {round(100*nPredict, 1)}", flush=True)
